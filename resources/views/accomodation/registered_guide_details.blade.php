            ...
            <div class='container'>
                <div class='card'>
                    <div class='card-header'>
                        <h3>Registered Guide Lists</h3>
                    </div>
                    <div class='card-body'>
                        <table class="table registered_guide-list w-100">
                            <thead>
                                <th>Registered Guide Name</th>
                                <th>Address</th>
...
<script>
    $('.registered_guide-list').DataTable({
        ...
        "ajax": {
            "url": "{{ route('DatatableRegisteredGuideList')}}",
            // "data": function(d) {
    ...
    $(document).on("click", ".delete", function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('DeleteRegisteredGuideData')}}",
                    headers: {
    ...
    $(document).on("click", ".edit", function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "{{route('GetEditRegisteredGuideData')}}",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: {
    ...
    $(document).on("submit", "#edit_form", function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "{{route('EditRegisteredGuideData')}}",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: $(this).serialize(),
